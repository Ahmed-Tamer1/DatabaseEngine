...
public class DBApp implements DBAppInterface, Serializable {
    ...
    static String DataPath = System.getProperty("user.dir") + "/src/main/resources/Data"; // change it according
    ...
    public void init() {
        try {
            // pathtxt = "C:/Users/Yasser Gallal/Desktop/DB2Project/Data";
                ...
                while (flag) {
                    ...
                    while (e.hasMoreElements()) {
                        ...
                        if (!sortHelper(tableName, key))
                            throw new DBAppException();
                        if (key.equals(clusteringKey) && !primaryKeyList.contains(value) && !primaryKeys.contains(value)) {
...
//        if (f == false) {
            ...
            if (file.exists()) {
                while (file.exists() && foundSearchUpdate == false) {
                    updateTableHelper(tableName, clusteringKeyValue, columnNameValue, s, i);
                    j = 1;
                    s = tableName + i + "overflow" + j + "ser";
                    pathS = DataPath + "/" + s;
                    file = new File(pathS);
...
//            throw new DBAppException();
    ...
    public void updateTableHelper(String tableName, String clusteringKeyValue,
                                  Hashtable<String, Object> columnNameValue, String s, int pageNumber)
            throws DBAppException, ParseException, IOException {
        ...
        if (clusteringKeyinfo.get(2).equals(" java.lang.Integer")) {
            ...
            for (int i = 0; i < deserializePage.size(); i++) {
                ...
                for (int j = 0; j < deserializePage.size(); j++) {
                    if (clusteringKeyinfo.get(1).equals(tupleSplite[j])) {
                        deserializePagePrimaryKeys.add(Integer.valueOf((String) (tupleSplite[j + 1])));
                    }
                }
            }
            ...
            if (index != -1) {
                String[] oldValues = ((deserializePage.get(index)).toString()).split(",");
                for (int i = 0; i < oldValues.length; i++) {
                    for (int j = 0; j < keySet.size(); j++) {
                        if (keySet.get(j).equals(oldValues[i])) {
                            oldValues[i + 1] = (String) columnNameValue.get(keySet.get(j));
                        }
                    }
                }
                for (int i = 0; i < oldValues.length; i++) {
                    newValue = newValue + oldValues[i];
                }
                deserializePage.set(index, newValue);
                foundSearchUpdate = true;
            } // else
            // System.out.println("Clustering Key Not Found ");
        } else if (clusteringKeyinfo.get(2).equals(" java.lang.String")) {
            ...
            for (int i = 0; i < deserializePage.size(); i++) {
                ...
                for (int j = 0; j < deserializePage.size(); j++) {
                    if (clusteringKeyinfo.get(1).equals(tupleSplite[j])) {
                        deserializePagePrimaryKeys.add(tupleSplite[j + 1]);
                    }
                }
            }
            ...
            for (int i = 0; i < deserializePage.size(); i++) {
                ...
                for (int j = 0; j < deserializePage.size(); j++) {
                    if (clusteringKeyinfo.get(1).equals(tupleSplite[j])) {
                        deserializePagePrimaryKeys.add(Double.parseDouble(tupleSplite[j + 1]));
                    }
                }
            }
            index = binarySearchDouble(deserializePagePrimaryKeys, 0, deserializePagePrimaryKeys.size(),
                    Double.parseDouble(String.valueOf(clusteringKeyValue)));
            if (index != -1) {
                String[] oldValues = (((deserializePage.get(index))).toString()).split(","); // split cannot resolve
                for (int i = 0; i < oldValues.length; i++) {
                    for (int j = 0; j < keySet.size(); j++) {
                        if (keySet.get(j).equals(oldValues[i])) {
                            oldValues[i + 1] = (String) columnNameValue.get(keySet.get(j));
                ...
                throw new DBAppException();
            // System.out.println("Clustering Key Not Found ");
        } else if (clusteringKeyinfo.get(2).equals(" java.lang.Date")) {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            Vector<java.util.Date> deserializePagePrimaryKeys = new Vector<java.util.Date>();
            for (int i = 0; i < deserializePage.size(); i++) {
                String tuple = (deserializePage.get(i)).toString();
                String[] tupleSplite = tuple.split(",");
                for (int j = 0; j < deserializePage.size(); j++) {
                    if (clusteringKeyinfo.get(1).equals(tupleSplite[j])) {
                        deserializePagePrimaryKeys.add(sdf.parse((tupleSplite[j + 1]).toString()));
                    }
                }
            }
            java.util.Date goal = sdf.parse(clusteringKeyValue.toString());
            index = binarySearchDate(deserializePagePrimaryKeys, goal);
            if (index != -1) {
                String[] oldValues = (((deserializePage.get(index))).toString()).split(",");
                for (int i = 0; i < oldValues.length; i++) {
                    for (int j = 0; j < keySet.size(); j++) {
    ...
    public void deleteFromTableHelper(String tableName, Hashtable<String, Object> columnNameValue, String s,
        ...
        for (int i = 0; i < deserializePage.size(); i++) {
            ...
            for (int j = 0; j < keySet.size(); j++) {
                for (int z = 1; z < tupleSplite.length; z++) {
                    if (tupleSplite[z].equals(keySet.get(j))) {
                        if (((Object) tupleSplite[z + 1]).equals(htValues[j])) {
                            delete = true;
                        } else {
                            delete = false;
                            break;
            ...
            if (delete == false)
                ...
                f = readClusteringKeyFromeMetadata(tableName);
                String clustringToDelete = "";
                for (int z = 0; z < tupleSplite.length; z++) {
                    {
                        if (tupleSplite[z].equals(f.get(1)))
                            clustringToDelete = tupleSplite[z + 1];
                    }
                    String k = tableName + "0.ser";
                    File file = new File(DataPath + "/" + k);
                    if (file.exists()) {
                        Vector<Object> h = deserializePage(k);
                        if (h.contains((Object) clustringToDelete)) {
                            h.removeElement((Object) clustringToDelete);
                        }
                    ...
                    if (file1.exists()) {
                        Vector<Object> h1 = deserializePage(k1);
                        if (h1.contains((Object) clustringToDelete)) {
                            h1.removeElement((Object) clustringToDelete);
                        }
                        serializeOverflow(tableName, 0, 0, h1);
                    }
    ...
    public static Bucket getBucket(int[] index, Object[] multiDimArr)
    ...
    public void gridIndexTraversal(Object[] a, int counter, String colName, String tableName, Object value, int sizeHelper) {
        this.counterTemp = counter;
        gridIndexTraversalHelper(a, counter, colName, tableName, value, sizeHelper);
    }

    public void gridIndexTraversalHelper(Object[] a, int counter, String colName, String tableName, Object value, int sizeHelper) {
...
//                      temp.add(this.counterTemp);
    ...
    public void CombinationRepetitionUtil(int[] chosen, int[] arr,
                                          int index, int r, int start, int end, Vector<Integer> temp) {
...
//            System.out.println(Arrays.toString(globalVectorOfCombinations.elementAt(globalVectorOfCombinations.size() - 1)));
...
// uses CombinationRepetitionUtil()
    ...
    public Iterator selectWithoutIndex(SQLTerm[] sqlTerms, String[] arrayOperators) throws DBAppException {
        Vector<Vector> selected = new Vector<>();
    ...
    public Vector selectWithoutIndexHelper(String tableName, Hashtable<String, Object> columnNameValue, String s,
                                           String[] sqlOperators, String[] keys, Object[] values, String operator) throws DBAppException, IOException {
...
//            if (finalResult.size() == 0) throw new DBAppException();
//            else result = finalResult.iterator();
...
//                }
    ...
    public static boolean checkHashTable(String tableName, Hashtable<String, Object> columnNameValue)
        ...
        if (file.exists()) {
            ...
            int z = 0;
            for (int i = 0; i < keySetarr.length; i++) {
                for (z = 0; z < columnNames.size() - 1; z++) {
...
//            int n = arr.length;
//        int r = 4;
//        dbApp.CombinationRepetition(dbApp.globalRangeArray, dbApp.globalRangeArray.length, r);
//        for (int i = 0; i < dbApp.globalVectorOfCombinations.size(); i++)
//            System.out.println(Arrays.toString(dbApp.globalVectorOfCombinations.elementAt(i)));

//            System.out.print(dbApp.rangeVector);
        ...
        max.put("id", "10");
        max.put("name", "zzzzzzzzzzzzzzzzz");
        max.put("gpa", "10.0");
        dbApp.init();
...
SQLTerm[] arrSQLTerms;
        ...
        while (resultSet.hasNext()) System.out.println(resultSet.next());
        for (int i=0;i<dbApp.globalVectorOfCombinations.size();i++)
            System.out.println(Arrays.toString(dbApp.globalVectorOfCombinations.elementAt(i)));
        }
